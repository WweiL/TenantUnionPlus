...
app.config.update(dict(
    ...
    DATABASE1=os.path.join(app.root_path, 'TenantUnionPlus1.db'),
...
def login():
    ...
    db = get_db()
    c = db.cursor()
    c.execute('SELECT count(*) FROM student1 WHERE NetID = ?', [user_netid])
    if c.fetchone() == 0:
        c.execute('INSERT INTO student1 (NetID) values ?', [user_netid])
    db.commit()
...
def profile(netid):
    ...
    db = get_db()
    c = db.cursor()
    c.execute('SELECT * FROM student1 WHERE NetID = ?', [netid])
...
@app.route('/house/profile/<building_name>/<room_num>')
def houseProfile(building_name,room_num):
    db = get_db()
    c = db.cursor()
    c.execute('SELECT * FROM room WHERE building_name = ? AND room_num=?', ([building_name, room_num]))
    user_profile = c.fetchone()
    db.commit()
    return render_template('house_profile.html',building_name=building_name,room_num=room_num)


def connect_db():
    """Connects to the specific database."""
    rv = sqlite3.connect(os.path.join(app.root_path, 'TenantUnionPlus.db'),)
